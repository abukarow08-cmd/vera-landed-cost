...
const V = {
  ...
  white: "#FFFFFF", offwhite: "#F0F4FA", black: "#0D0D0D",
  surface: "#F7F9FC", surface2: "#EDF1F8", border: "#D0DCF0",
  muted: "#7A90B0", text: "#0D1A2E",
};
...
const init = () => PRODUCTS.reduce((acc, p) => {
  acc[p.id] = { units: 100, unitCost: p.defaultCost, shippingShare: 50, dutyRate: p.defaultDuty, vatRate: 25, insuranceRate: 0.8, handlingShare: 20, lastMileShare: 10, enabled: true };
  return acc;
}, {});
...
function ProductCard({ product, data, onChange }) {
...
export default function VeraLandedCost() {
  ...
  const handleChange = useCallback((pid, key, val) => { setData(prev => ({ ...prev, [pid]: { ...prev[pid], [key]: val } })); }, []);
    ...
    <div style={{ background:V.surface, minHeight:"100vh", color:V.text, fontFamily:"'DM Mono','Courier New',monospace", paddingBottom:60 }}>
      ...
      <div style={{ maxWidth:960, margin:"0 auto", padding:"0 16px" }}>
        ...
        <div style={{ display:"flex", justifyContent:"flex-end", padding:"8px 0 0" }}>
          <button onClick={() => { const allOn=PRODUCTS.every(p=>data[p.id].enabled); setData(prev => { const n={...prev}; PRODUCTS.forEach(p=>{n[p.id]={...n[p.id],enabled:!allOn};}); return n; }); }} style={{ background:V.white, border:`1px solid ${V.border}`, borderRadius:5, color:V.muted, cursor:"pointer", fontSize:9, padding:"5px 12px", letterSpacing:"0.15em", fontFamily:"'DM Mono',monospace" }}>{PRODUCTS.every(p=>data[p.id].enabled)?"COLLAPSE ALL":"EXPAND ALL"}</button>
        </div>
        {PRODUCTS.map(p => <ProductCard key={p.id} product={p} data={data[p.id]} onChange={(k,v)=>handleChange(p.id,k,v)}/>)}
        {calcs.length > 1 && (
        ...
        <button onClick={exportPDF} style={{ marginTop:24, width:"100%", background:`linear-gradient(90deg,${V.blue1},${V.blue2},${V.blue3})`, border:"none", borderRadius:8, color:V.white, fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontStyle:"italic", fontSize:14, letterSpacing:"0.2em", textTransform:"uppercase", padding:"14px 28px", cursor:"pointer", boxShadow:"0 4px 16px rgba(43,107,196,0.35)" }}>↓ Download PDF Report</button>
      </div>
    </div>
  );
}
